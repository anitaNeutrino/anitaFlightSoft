...
/* Edited by RJN to add function declaration */
static word build_control(struct conf_blk_320 *c_blk, struct scan_array *sa_ptr);
...
*/
    ...
    output_word( c_blk->nHandle, &c_blk->brd_ptr->cntl_reg,
...
*/
      ...
      if((input_word( c_blk->nHandle, &c_blk->brd_ptr->cntl_reg ) & CTRIG ) != 0)/* old data may be present */
        *buff_ptr = input_word( c_blk->nHandle, &c_blk->brd_ptr->ai_reg );	 /* read data register to clear flag */

      for(i = 0; i < c_blk->average; i++)
      {
        if(c_blk->trigger == ETRIG)		/* check external trigger */
	  while((input_word( c_blk->nHandle, &c_blk->brd_ptr->cntl_reg ) & CTRIG) == 0); /* wait for trigger */
        else
          output_word( c_blk->nHandle, &c_blk->brd_ptr->strt_reg, 0xFFFF ); /* start conversion (recommended value) */
	for(temp = 0; temp != 110; temp++)	/* longer delay loop-JJB */
	/* replace delay loop with usleep
	   ...but it appears minimum delay is one time slice*/
	/*usleep(10);*/
	/*        for(temp = 0; temp != 11; temp++) */	/* no flag to poll, kill some time */
	   input_word( c_blk->nHandle, &c_blk->brd_ptr->cntl_reg );

/*
        Read and sum the data samples
*/

        sum_data += (float)(input_word( c_blk->nHandle, &c_blk->brd_ptr->ai_reg ) & c_blk->data_mask); /* read data register */
      }
...
*/
      ...
      *buff_ptr++ = (word)(sum_data/(float)c_blk->average);
      ...
      output_word( c_blk->nHandle, &c_blk->brd_ptr->cntl_reg, control_reg );	/* set in new control register */
    }
}
...
static word build_control(c_blk, sa_ptr)
