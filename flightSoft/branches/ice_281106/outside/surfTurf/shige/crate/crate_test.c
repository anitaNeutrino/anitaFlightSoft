...
RETURN_CODE cntl_set_SURF(HANDLE PlxHandle, SURF_cntl_action action) {
  ...
  switch (action) {
  case ClrAll : gpio_v = base_v | clr_all  ; break ; 
...
int init_device(HANDLE *PlxHandle, DEVICE_LOCATION *Device) {
  ...
  /* Initialize TURFIO */
  i = DeviceNum-1 ; /* this is for turf furtherst away from CPU than any surf.
		       PlxLinux counts from CPU, i.e. closest PCI card is index 0. */
  if (PlxPciDeviceFind(&Device[i], &i) != ApiSuccess) 
    return -1 ;
  if (verbose) 
    printf("init_device: device found, %.4x %.4x [%s - bus %.2x  slot %.2x]\n",
...
void write_data(char *directory, unsigned short wv_data[N_SURF][N_CHN][],unsigned long trigtime) {
  ...
  sprintf(f_name, "%s/surf_data.%lu", directory, trigtime) ;
...
void write_turf(char *directory, TURFIO_struct *data_turf) {
  ...
  sprintf(f_name, "%s/turf_data.%lu", directory, data_turf->time) ;
...
/* 	 Plxstate.PciMainInt, Plxstate.IopToPciInt, Plxstate.IopToPciInt_2) ; */
  ...
  while ((n++ < n_ev) || (n_ev == 0)) {  
    ...
    tmo=0;
    while (!((tmpGPIO=PlxRegisterRead(PlxHandle[n_dev-2], PCI9030_GP_IO_CTRL, &rc)) & 0x800) &&
	...
	printf("Failed to set EvNoD on SURF %d\n",k);
      ...
      // Wait for Lab_F
      ftmo=0;
      while(!(PlxRegisterRead(PlxHandle[k], PCI9030_GP_IO_CTRL, &rc) & 0x4000) &&
	    (++ftmo<N_FTMO)){
	usleep(1000);
      }
	...
	for (i=0 ; i<N_CHN ; i++) {
	  ...
	  else data_array[k][i][j] = d_in & 0xfff ;
	}    
      }
...
/*       if (verbose) printf(" data(0/0) = %x \n", d_in) ; */
    ...
    if(!surfonly){
      mean_dt=mean_dt*(n-1)/n+(float)data_turf.interval/n;
	...
	mean_rms[k][chip[k]][j]=mean_rms[k][chip[k]][j]*(n-1)/n+sqrt(sum/(N_SCA-7))/n;
      }
    // Display mean rms
    for(k=0;k<n_dev-1;++k){
      printf("Board %d RMS\n",k);
