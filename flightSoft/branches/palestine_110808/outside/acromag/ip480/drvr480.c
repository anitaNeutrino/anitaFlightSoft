...
*/
    ...
    struct cblk480 c_block;    /* configuration block */
...
*/
    ...
    for( i = 0; i < 7; i++ )
    {
        c_block.m_CounterConstant[i] = 0xffff;     /* constant registers */
        c_block.m_Mode[i] = 0;                     /* the counter mode */
        c_block.m_OutputPolarity[i] = FALSE;       /* output polarity */
        c_block.m_InputPolarity[i] = FALSE;        /* input polarity */
        c_block.m_Trigger[i] = 0;                  /* triggering internal/external */
        c_block.m_CounterSize[i] = 0;              /* counter size */
        c_block.m_ClockSource[i] = 0;              /* clock source */
        c_block.m_WDLoad[i] = 0;                   /* watchdog timer internal/external loaded */
        c_block.m_InterruptEnable[i] = FALSE;      /* interrupt enable */
        c_block.m_Debounce[i] = DebounceOff;       /* Debounce disabled only 1 & 2 used */
    }
    c_block.m_InterruptVector = VECTOR;            /* interrupt vector */
    c_block.event_status = 0;                      /* clear event status */
    c_block.counter_num = 1;
    c_block.num_chan = 2;                          /* default to 2 channel board */
...
*/
    ...
    while(!finished)
    {
      ...
      printf(" 5. Read Module I.D./Display Event Status\n");
...
*/
	...
	break;
        ...
        case 4: /* Reset board */
	        ...
	        output_word( c_block.nHandle, (word *)(c_block.brd_ptr + ResetRegister), Reset);
        break;
		...
		printf("\n>>> ERROR: BOARD ADDRESS NOT SET <<<\n");
                ...
                readstat480(&c_block); /* read board status */
        break;


        case 6: /* counter number */
            selectcnt480(&c_block);
        break;
	    ...
	    {
	        ConfigureCounterTimer480(&c_block, c_block.counter_num);
        /*      printf("\n\nControl Word: %03x\n",build_control480(&c_block,c_block.counter_num));*/
	    }
        break;
		...
		hflag = 0;
		DisableInterrupt480(&c_block,c_block.counter_num);
        break;
	    ...
	    else
	    {
               ...
               cont_reg = input_word( c_block.nHandle, (word *)(c_block.brd_ptr + CounterControl1));
               printf("%04X ",cont_reg);
               cont_reg = input_word( c_block.nHandle, (word *)(c_block.brd_ptr + CounterControl2));
               printf("%04X ",cont_reg);
               cont_reg = input_word( c_block.nHandle, (word *)(c_block.brd_ptr + CounterControl3));
               printf("%04X ",cont_reg);
               cont_reg = input_word( c_block.nHandle, (word *)(c_block.brd_ptr + CounterControl4));
               printf("%04X ",cont_reg);
               cont_reg = input_word( c_block.nHandle, (word *)(c_block.brd_ptr + CounterControl5));
               printf("%04X ",cont_reg);
               cont_reg = input_word( c_block.nHandle, (word *)(c_block.brd_ptr + CounterControl6));
               printf("%04X \n",cont_reg);
            }
        break;
	    ...
	    else
	    {
              ...
              for(i = 1; i < 7; i++)
              {
               ...
               {
               ReadCounter480(&c_block, i, &ulong_value);
		...
		printf("\n>>> ERROR: BOARD ADDRESS NOT SET <<<\n");
	        ...
	        WriteCounterConstant480(&c_block, c_block.counter_num);
        break;
		...
		printf("\n>>> ERROR: BOARD ADDRESS NOT SET <<<\n");
	        ...
	        StopCounter480(&c_block, c_block.counter_num);
        break;
