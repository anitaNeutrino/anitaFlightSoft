...
*/
   ...
   status = 0;
   w_address = s_address = (unsigned short *) address2;
...
*/
   ...
   for(l = 0; l < 1024; l++)           /* check for 0 */
   {
     ...
     {
       status = -1;
       sprintf(buf,"FAIL\nIP%c memory %08lX read %04X  Expected 0000 ",c_blk->slotLetter,(unsigned long)&s_address[l],dut_data);
       printf(buf);
     }
   }
...
*/
   ...
   if(status == 0)
   {
     ...
     for(l = 0; l < 1024; l++)         /* check for 010A */
     {
       ...
       {
         status = -2;
         sprintf(buf,"FAIL\nIP%c memory %08lX read %02X  Expected 010A ",c_blk->slotLetter,(unsigned long)&s_address[l],dut_data);
         printf(buf);
       }
     }
   }
...
*/
   ...
   if(status == 0)
   {
     ...
     for(l = 0; l < 1024; l++)         /* check for 0x0205 */
     {
       ...
       {
         status = -3;
         sprintf(buf,"FAIL\nIP%c memory %08lX read %02X  Expected 0205 ",c_blk->slotLetter,(unsigned long)&s_address[l],dut_data);
         printf(buf);
       }
     }
   }
