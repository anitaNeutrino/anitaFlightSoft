...
*/
    ...
    int i;			/* loop index */
    word word_value;
    struct cblk482 c_block482;	/* configuration block */
...
*/
    c_block482.bCarrier = FALSE;	/* indicate no initialized and set up yet */
    c_block482.bInitialized = FALSE; /* indicate not ready to talk */
    c_block482.nHandle = 0;	/* make handle to a closed board */
    ...
    c_block482.m_IOSClock = 0;                  /* 8MHZ clock from carrier */
...
*/
    ...
    while(!finished)
    {
      printf("\nIOS482 Counter/Timer Library Demonstration Rev. A\n\n");
...
*/
				if(GetIOSAddress(c_block482.nHandle, c_block482.slotLetter, &addr) != S_OK)
				{
...
*/
    ...
    while(!finished)
    {
        ...
        status = GetIOSClock(c_blk, &bool_value);
	...
	case 2: /* board address */
        ...
        case 12: /* IOS clock select */
            ...
            else
			{
               ...
               SetIOSClockControl(c_blk->nHandle, c_blk->slotLetter, bool_value);

               /* call IOSmodule API to select clock speed */
               SetIOSClock(c_blk, bool_value);
			}
            break;
...
{+D}
    ...
    MODULE NAME:    trig - Software trigger for ios482.
    ...
    ABSTRACT:       Routine triggers counter/timers on the ios482.
...
/*
{+D}
    SYSTEM:         ios482 Software

    FILENAME:       ios482.c

    MODULE NAME:    stop_trig - Software stop trigger for ios482.
    ...
    ABSTRACT:       Routine stop triggers counter/timers on the ios482.
...
{+D}
    ...
    MODULE NAME:    selectcnt - Select counter for ios482.
    ...
    ABSTRACT:       Routine selects counter/timers on the ios482.
...
{+D}
    ...
    MODULE NAME:    selectcon - Select counter constant for ios482.
    ...
    ABSTRACT:       Routine selects counter constants for the ios482.
