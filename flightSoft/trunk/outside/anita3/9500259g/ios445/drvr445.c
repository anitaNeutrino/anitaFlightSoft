...
*/
    ...
    s_block445.bInitialized = FALSE;	/* indicate not ready to talk */
...
*/      
	...
	while(!finished) {
		...
		printf("\n\nIOS445 Library Demonstration  Version A\n\n");
...
*/
				if(GetIOSAddress(s_block445.nHandle, s_block445.slotLetter, &addr) != S_OK)
				{
...
*/
	...
	printf("\nIOS Model Number:       %x",(byte)s_blk->id_prom[5]);
...
reentry:
	...
	printf(" 4. Display Entire EEPROM\n");
	printf("\nselect: ");
	scanf("%d",&item);
	...
	case 4: /* read/display entire EEPROM */
		cycle = 0;
		for( i = 0; i <= 0xFC; i += 4)
		{
	...
	if( cycle == 0 )
	    printf("Quad byte aligned address: %02X     EEPROM data: %08X", i, VPDData);
