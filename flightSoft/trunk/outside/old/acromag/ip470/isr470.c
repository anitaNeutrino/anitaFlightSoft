...
struct conf_blk_470* cblk;	      /*  configuration block */
...
*/
    ...
    cblk = (struct conf_blk_470*)hData->hd_ptr;
    saved_bank = bank_select(BANK1, cblk); /* set & save bank select */
        
    i_stat = input_byte(&cblk->brd_ptr->port[6].b_select); /* interrupt status */
    for(i = 0; i < 6; i++)      /* check ports 0-5 for an interrupt pending */
    {
      ...
      if((p_mask & i_stat) != 0) /* port with interrupt pending? */
      {
        ...
        cblk->event_status[i] |= input_byte(&cblk->brd_ptr->port[i].b_select); /* interrupt sense */

	/* write 0s to clear the interrupting bits */
        output_byte(&cblk->brd_ptr->port[i].b_select,(~(cblk->event_status[i]))); /* write 0 to clear the interrupting bit */
...
*/
        ...
        output_byte(&cblk->brd_ptr->port[i].b_select, 0xFF); /* re-enable sense inputs */ 
      }
    }
