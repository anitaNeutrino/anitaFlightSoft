...
*/
    ...
    struct cblk231 c_block;    /* configuration block */
...
*/
    ...
    c_block.mode = TM;			/* mode */
    c_block.bit_constant = CON16;	/* constant for data correction */
    c_block.ideal_buf = &ideal_data[0];	/* ideal value */
    c_block.off_buf = &off_data[0];	/* offset buffer start */
    c_block.gain_buf = &gain_data[0];	/* gain buffer start */
    c_block.cor_buf = &cor_data[0];	/* corrected */
...
*/
				...
				{	
	                c_block.brd_ptr = (struct map231 *)addr;
	...
	break;
        ...
        case 7: /* Read Calibration Coefficients and I.D. */
	      ...
	      readstat231(&c_block);	/* read and display status */
...
hloop1:
		    ...
		    wro231(&c_block, channel, ideal_data[channel]);
	          break;
...
vloop1:
		    ...
		    ftemp =((float)c_block.bit_constant * volts) / 20.0;
		    if( ftemp < 0)
		      ftemp -= 0.5;
		    else
		      ftemp += 0.5;
...
hloop2:
		    ...
		    scanf("%x", &temp);
		    ideal_data[channel] = (word)temp;
		    cd231(&c_block, channel);	/* correct data */
		    wro231(&c_block, channel, cor_data[channel]);
	          break;
...
vloop2:
            ...
            else
	    {
	      finished2 = 0;
	      channel = 0;
	      i = c_block.mode;	/* save mode */
	      c_block.mode = TM;	/* set mode */
	      mode_select231(&c_block);
