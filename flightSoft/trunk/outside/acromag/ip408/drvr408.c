...
*/
    ...
    struct sblk408 s_block;  /* allocate status param. blk */
    struct cblk408 c_block;  /* configuration block */
...
*/
    ...
    c_block.int_status = 0;     /* pending interrupts to clear */
    c_block.enable = 0;         /* interrupt enable (per bit) */
    c_block.polarity = 0;       /* interrupt polarity */
    c_block.type = 0;           /* interrupt type */
    c_block.vector = VECTOR;    /* interrupt vector base */
    ...
    s_block.vector = 0;         /* interrupt vector base */
    ...
    c_block.sblk_ptr = (struct sblk408*)&s_block;
...
*/
				...
				{	
			        c_block.brd_ptr = (struct map408 *)addr;
...
*/
                ...
                if( input_byte ( c_block.nHandle,&c_block.brd_ptr->sts_reg ))
		...
		else
		{
                    if( hflag == 0 )
                        ...
                        cnfg408(&c_block); /* configure the board */
                }
            }
        break;
            ...
            else
	    {
                ...
                scanf("%d",&point);
                status = rpnt408(&c_block,port,point);
                if(status == -1)
            ...
            else
	    {
                ...
                scanf("%d",&point);
                status = robt408(&c_block,port,point);
                if(status == -1)
                  printf("\n>>> ERROR: PARAMETER OUT OF RANGE <<<\n");
                else
                  printf("\nValue of port %d point %d: %X\n",port,point,status);
            ...
            else
	    {
                ...
                scanf("%x",&val);
                status = wpnt408(&c_block,port,point,val);
                if(status == -1)
                    printf("\n>>> ERROR: PARAMETER OUT OF RANGE <<<\n");
            }
            break;
            ...
            else
	    {
                ...
                scanf("%x",&val);
                status = wprt408(&c_block,port,val);
                if(status == -1)
                    printf("\n>>> ERROR: PARAMETER OUT OF RANGE <<<\n");
            }
            break;
