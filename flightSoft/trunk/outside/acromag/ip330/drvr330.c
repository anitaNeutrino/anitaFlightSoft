...
*/
    ...
    struct cblk330 c_block;    /* configuration block */
...
*/
    ...
    c_block.range = RANGE_5TO5;         /* default +-5 V */
    c_block.acq_mode = DI_SELECT;       /* mode */
    c_block.scan_mode = DISABLE;	/* scan mode */
    c_block.s_channel = 0;              /* start channel */
    c_block.e_channel = 0xf;            /* end channel*/
    c_block.data_format = SB_SELECT;	/* A/D data format */
    c_block.timer_ps = 0xff;            /* prescaler */
    c_block.conv_timer = 0x15;          /* counter */
    c_block.timer_en = TIMER_OFF;	/* timer off */
    c_block.trigger = TO_SELECT;	/* trigger I/O is output */
    c_block.int_mode = INT_DIS; 	/* disable interrupt mode */
    c_block.vector = VECTOR;            /* interrupt vector */
    c_block.s_raw_buf = &raw_data[0];	/* raw buffer start */
    c_block.s_az_buf = &az_data[0];	/* auto zero buffer start */
    c_block.s_cal_buf = &cal_data[0];   /* calibration buffer start */
    c_block.s_cor_buf = &cor_data[0];   /* corrected buffer start */

    c_block.new_data[0] = 0;            /* new data flags */
    c_block.new_data[1] = 0;            /* new data flags */
    c_block.missed_data[0] = 0;         /* missed data flags */
    c_block.missed_data[1] = 0;         /* missed data flags */
    c_block.control = 0;                /* control register used by read only*/
...
*/
    ...
    while(!finished)
    {
      ...
      printf(" 6. N/A                                  16. Display Corrected Data In Volts\n");
...
*/
				...
				{	
			                c_block.brd_ptr = (struct map330 *)addr;
	...
	case 4: /* set up configuration block parameters */
		...
		printf("\n>>> ERROR: BOARD ADDRESS NOT SET <<<\n");
                ...
                readstat330(&c_block); /* read board status */
	    break;
	   ...
	   DisableInterrupts(c_block.nHandle);
        ...
        case 8: /* read auto zero values */
            ...
            calibrate330(&c_block, AZ_SELECT);   /* get auto-zero values */
            break;
        ...
        case 9: /* read calibration data values */
            ...
            calibrate330(&c_block, CAL_SELECT);  /* get calibration values */
            break;
        ...
        case 10: /* read raw input data values */
            ...
            if(hflag == 0 && c_block.int_mode != 0)
            {
              printf("\n>>> ERROR: NO INTERRUPT HANDLERS ATTACHED <<<\n");
              break;
            }
 
            convert330(&c_block);                /* convert the board */
            if(c_block.missed_data[0] != 0)   /* check for missed data */
               printf("\n<<< missed data >>> register 0 = %0x",c_block.missed_data[0]);

            if(c_block.missed_data[1] != 0)   /* check for missed data */
               printf("\n<<< missed data >>> register 1 = %0x",c_block.missed_data[1]);
             break;

        case 11: /* calculate corrected data value for multi channel*/
            mccd330(&c_block);       /* correct input data */
	    break;
...
/*
    ...
    for channels 10-1F
*/
              if(c_block.acq_mode == DI_SELECT && i > 0xf)
                j &= 0xf;

              printf("%12.6f ",((double)cor_data[i] / (double)CON16 * s + z) /
                    (double)(1 << get_gain(&c_block, j)));
	    }
            break;
...
*/
    if(c_block.bInitialized)
    {
      c_block.scan_mode = 0;      /* stop scan */
      c_block.int_mode = 0;       /* disable interrupts */
      cnfg330(&c_block);          /* configure the board */
    }
