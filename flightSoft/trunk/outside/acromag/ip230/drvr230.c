...
*/
    ...
    int i;			/* loop index */
    int channel;
    struct cblk230 c_block;    /* configuration block */
...
*/
    ...
    c_block.mode = 0;			/* mode */
    c_block.bit_constant = CON16;	/* constant for data correction */
...
*/
				...
				{	
	                c_block.brd_ptr = (struct map230 *)addr;
					c_block.bInitialized = TRUE;
				}
			}
		}
            printf("\nPlease Wait, Reading Calibration Coefficients....\n");
           rcc230(&c_block, num_chan); /* read the calibration coef. into an array */
	break;
        ...
        case 5:				/* Control Register Setup and Range Selection */
	        ...
	        cont_reg(&c_block, hflag);	/* control register config routine */
            break;
...
vloop1:
             ...
             if(c_block.range[channel] == BIPOLAR_5)
               ideal_volt_span = VOLT_SPAN;

             if(c_block.range[channel] == UNIPOLAR_10)
               ideal_volt_span = VOLT_SPAN;
  	     ...
  	     wro230(channel,(short)(ideal_data[channel]), &c_block);
	    }
        break;
...
vloop2:
             ...
             if(c_block.range[channel] == BIPOLAR_10)
             ...
             c_block.ideal_buf[channel] = (short)ideal_data[channel]; 

	     cd230(&c_block, channel);	/* correct data */ 

             cor_data[channel] = (word)c_block.cor_buf[channel];

	     wro230(channel, c_block.cor_buf[channel], &c_block);
	    }
            break;
	    ...
	    for(i = 0; i != num_chan; i++)
          {
           if(c_block.range[i] == BIPOLAR_5)
	      printf("     %04x",(c_block.coef_bufb5[i][1] & 0xFFFF));
           if(c_block.range[i] == BIPOLAR_10)
	      printf("     %04x",(c_block.coef_bufb10[i][1] & 0xFFFF));
           if(c_block.range[i] == UNIPOLAR_10)
	      printf("     %04x",(c_block.coef_bufu10[i][1] & 0xFFFF));
          }
	    ...
	    for(i = 0; i != num_chan; i++)
          {
           if(c_block.range[i] == BIPOLAR_5)
	      printf("     %04x",(c_block.coef_bufb5[i][0] & 0xFFFF));
           if(c_block.range[i] == BIPOLAR_10)
	      printf("     %04x",(c_block.coef_bufb10[i][0] & 0xFFFF));
           if(c_block.range[i] == UNIPOLAR_10)
	      printf("     %04x",(c_block.coef_bufu10[i][0] & 0xFFFF));
          }
        ...
        case 11:      /* display the control and timer registers */
			...
			(int)input_word( c_block.nHandle, (word*)&c_block.brd_ptr->cont_reg ));
             if(model == 3 || model == 4)
             {
               printf("Timer Prescaler Register:       %02X\n",
			(int)input_word( c_block.nHandle, (word*)&c_block.brd_ptr->tim_pres ));
               printf("Conversion Timer Register:      %04X\n",
			(int)input_word( c_block.nHandle, (word*)&c_block.brd_ptr->conv_tim ));
             }
            }
           break;
	      ...
	      the corresponding voltage for the range. */
			...
			v = rad * half_span + midpoint;
                        ...
                        if( temp < 0.0)		/* round */
                       	...
                       	c_block.ideal_buf[channel] = (word)temp;

			if(c_block.range[channel] == UNIPOLAR_10)
			        c_block.ideal_buf[channel] ^= 0x8000;
 
			cd230(&c_block, channel);

	                wav_mem( 1, i, c_block.cor_buf[channel], &c_block);
	      }
	    }
		      break;
		...
		gloop6:
				...
				for( i = MIN_WAVEFORM_ADDRESS; i <= MAX_WAVEFORM_ADDRESS; i++ )
				{
		       			wav_mem( 1, i, (short)midpoint, &c_block);
				}
		...
		try_again2:
           		...
           		printf("Enter Data:\n");
           		scanf("%x", &data);
           		wav_mem( 1, address, data, &c_block);
	      		break;
	           }
                }
            }
...
try_again:
           ...
           data = wav_mem( 0, address, 0x0000, &c_block);
...
tloop1:
        ...
        case 15: /* Write to the Timer Prescaler */
			...
			(int)input_word( c_block.nHandle, (word*)&c_block.brd_ptr->tim_pres ) & 0xff);
tloop2:
...
*/
    if(!c_block.bInitialized)    /* module address was set */
      soft_reset(&c_block);      /* reset board */
