...
*/
    ...
    struct sblk409 s_block;  /* allocate status param. blk */
    struct cblk409 c_block;  /* configuration block */
...
*/
    ...
    for( i = 0; i < 2; i++)
    {
	c_block.direction[i] = 0;      /* direction */
	c_block.int_status[i] = 0;     /* pending interrupts to clear */
	c_block.enable[i] = 0;         /* interrupt enable (per bit) */
	c_block.polarity[i] = 0;       /* interrupt polarity */
	c_block.type[i] = 0;           /* interrupt type */

	s_block.direction[i] = 0;      /* direction */
	s_block.int_status[i] = 0;     /* pending interrupts to clear */
	s_block.enable[i] = 0;         /* interrupt enable (per bit) */
	s_block.polarity[i] = 0;       /* interrupt polarity */
	s_block.type[i] = 0;           /* interrupt type */
    }
    ...
    c_block.sblk_ptr = (struct sblk409*)&s_block;
...
*/
				...
				{	
			                c_block.brd_ptr = (struct map409 *)addr;
...
*/
		...
		if( input_word( c_block.nHandle, (word*)&c_block.brd_ptr->sts_reg[0] ) || ( input_word( c_block.nHandle, (word*)&c_block.brd_ptr->sts_reg[1] ) && 0x00FF))
		      printf(">>> ERROR: INTERRUPTS ARE PENDING <<<\n");
		else
		{
		   if( hflag == 0 && ( c_block.enable[0] || c_block.enable[1] ))
			printf(">>> ERROR: INTERRUPT HANDLER NOT ATTACHED <<<\n");
		   else
		        cnfg409(&c_block); /* configure the board */
		}
	    }
	    break;
	...
	case 6:     /* read board status command */
		...
		psts409(&c_block); /* read board status */
	    break;
	...
	case 9: /* Read Digital Input Point */
		...
		scanf("%d",&point);
		status = rpnt409(&c_block,port,point);
		if(status == -1)
	...
	case 11: /* Write Digital Point */
		...
		scanf("%x",&val);
		status = wpnt409(&c_block,port,point,val);
		if(status == -1)
		    printf("\n>>> ERROR: PARAMETER OUT OF RANGE <<<\n");
	    }
	    break;
	...
	case 12: /* Write Digital Port */
		...
		scanf("%x",&val);
		status = wprt409(&c_block,port,val);
		if(status == -1)
		    printf("\n>>> ERROR: PARAMETER OUT OF RANGE <<<\n");
	    }
	    break;
...
*/
    if(c_block.bInitialized)		/* module address was set */
    {
      ...
      c_block.int_status[0] = 0xffff;   /* pending interrupts to clear */
      c_block.int_status[1] = 0x00ff;   /* pending interrupts to clear */
      c_block.enable[0] = c_block.enable[1] = 0; /* interrupt enable (per bit) */
      cnfg409(&c_block);		/* configure the board */
    }
