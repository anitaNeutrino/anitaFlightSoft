...
typedef struct _DMA_TRANSFER_ELEMENT
{
    union
    {
        ...
        U32      SourceAddr;            // ** no longer supported **
    } u;
    U32          PciAddrHigh;           // Upper 32-bits of PCI address
    U32          LocalAddr;             // Local bus address
    U32          DestAddr;              // ** no longer supported **
    U32          TransferCount;         // Number of bytes to transfer
    U32          NextSglPtr;            // ** no longer supported **
    unsigned int PciSglLoc         :1;  // ** no longer supported **
    unsigned int LastSglElement    :1;  // ** no longer supported **
