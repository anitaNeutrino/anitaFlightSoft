...
PedSubbedEventBody_t thePedSubbedBody;

int main(int argc, char** argv) {
    char dirName[FILENAME_MAX];
    char outDirName[FILENAME_MAX];
    char realName[FILENAME_MAX];
    char outRealName[FILENAME_MAX];
    char outRealDir1[FILENAME_MAX];
    char outRealDir2[FILENAME_MAX];
...
//    makeDirectories(ACQD_EVENT_LINK_DIR);
    ...
    char bigOutEventFileName[FILENAME_MAX];
    char bigOutHeaderFileName[FILENAME_MAX];
    ...
    if(argc<3) {
	printf("Usage:\t%s <inputDir> <outputDir>\n",basename(argv[0]));
	return 0;
    }
    strncpy(dirName,argv[1],FILENAME_MAX-1);
    strncpy(outDirName,argv[2],FILENAME_MAX-1);


    printf("Input Dir: %s\n",dirName);
    printf("Output Dir: %s\n",outDirName);
// create output directory, complain and exit if it exists    
    if (mkdir(outDirName,0744)!=0){
	 printf("slac2ice: output directory already exists\n");
	 exit(-1);
    }
//    for(eventNum=0;1;eventNum+=10000) {
	...
	sprintf(outRealName,"%s/ev%d/ev%d/",outDirName,dirNum,subDirNum);
	printf("Converting: %s %s\n",realName,outRealName);
	sprintf(outRealDir1,"%s/ev%d",outDirName,dirNum);
	printf("Creating directory: %s\n",outRealDir1);
	if (mkdir(outRealDir1,0744)!=0) {printf("outRealDir1 failure\n");exit(-1);}
	sprintf(outRealDir2,"%s/ev%d/ev%d",outDirName,dirNum,subDirNum);
	printf("Creating directory: %s\n",outRealDir2);
	if (mkdir(outRealDir2,0744)!=0) {printf("outRealDir2 failure\n");exit(-1);}
...
//	exit(0);
	...
	for(count=0;count<numLinks;count++) {
	    ...
	    sprintf(bigOutEventFileName,"%s/psev_%d.dat.gz",outRealName,doingEvent);
	    sprintf(bigOutHeaderFileName,"%s/hd_%d.dat.gz",outRealName,doingEvent);
//	printf("%s\n",bigHeaderFileName);
//	printf("%s\n",bigEventFileName); 
	    inputEvent=gzopen(bigEventFileName,"rb");
	    inputHead=gzopen(bigHeaderFileName,"rb");
	    outputEvent=gzopen(bigOutEventFileName,"wb");
	    outputHead=gzopen(bigOutHeaderFileName,"wb");
		...
		subtractPedsFromBufferedFile(&theBody,&thePedSubbedBody,
				     "slacPedsNewFormat.dat",0);
		if(retVal!=0 ||
