...
int main (int argc, char *argv[])
{
    ...
    char prioritizerdEventBackupDir[FILENAME_MAX];
    char prioritizerdLinkDirBase[FILENAME_MAX];
...
/*     eString = configErrorString (status) ; */
	...
	strncpy(eventdEventLinkDir,kvpGetString("eventdEventLinkDir"),
		FILENAME_MAX-1);
	strncpy(prioritizerdEventDir,kvpGetString("prioritizerdEventDir"),
		FILENAME_MAX-1);
	strncpy(prioritizerdEventBackupDir,kvpGetString("prioritizerdEventBackupDir"),FILENAME_MAX-1);
	strncpy(prioritizerdLinkDirBase,kvpGetString("prioritizerdLinkDirBase"),FILENAME_MAX-1);
    }
    
    makeDirectories(prioritizerdEventDir);
    makeDirectories(prioritizerdEventBackupDir);
    for(priority=0;priority<=9;priority++) {
       
	sprintf(tempFilename,"%s/%d/",prioritizerdLinkDirBase,priority);
 	makeDirectories(tempFilename); 
    }
...
/* 	    printf("%s\n",eventLinkList[count]->d_name); */
	    sprintf(hdFilename,"%s/%s",eventdEventLinkDir,
		    eventLinkList[count]->d_name);
	    ...
	    priority=setPriority(&theEventHeader,&theEventBody);
	    syslog(LOG_INFO,"Got Priority %d for event %d",theEventHeader.priority,theEventHeader.eventNumber);
	    writeOutputFiles(&theEventHeader,&theEventBody,
			     prioritizerdEventDir,prioritizerdEventBackupDir,
			     prioritizerdLinkDirBase,probWriteOut9);
	    /* Delete input */
