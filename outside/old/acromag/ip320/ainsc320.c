...
*/
    ...
    output_word( c_blk->nHandle, &c_blk->brd_ptr->cntl_reg, control );	/* write out control register */

    sum_data = 0.0;

    if((input_word( c_blk->nHandle, &c_blk->brd_ptr->cntl_reg ) & CTRIG) != 0) /* old data may be present */
      temp = input_word( c_blk->nHandle, &c_blk->brd_ptr->ai_reg );	       /* read data register to clear flag */
	...
	while((input_word( c_blk->nHandle, &c_blk->brd_ptr->cntl_reg ) & CTRIG) == 0); /* wait for trigger */
      ...
      /* delay loop here replaced with usleep */
      usleep(10);
