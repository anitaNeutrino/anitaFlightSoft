...
*/
	...
	struct sblk445 s_block;  /* allocate status param. blk */
...
*/
				...
				{	
	                s_block.brd_ptr = (struct map445 *)addr;
		...
		case 5: /* Read Digital I/O Point */
			...
			else
			{
				...
				scanf("%d",&point);
				status = rpnt445(&s_block,port,point);
				if(status == -1)
		...
		case 7: /* Write Digital I/O Point */
			...
			else
			{
				...
				scanf("%x",&val);
				status = wpnt445(&s_block,port,point,(byte)val);
				if(status == -1)
					printf("\n>>> ERROR: PARAMETER OUT OF RANGE <<<\n");
			}
			break;
		...
		case 8: /* Write Digital I/O Port */
			...
			else
			{
				...
				scanf("%x",&val);
				status = wprt445(&s_block,port,(byte)val);
				if(status == -1)
					printf("\n>>> ERROR: PARAMETER OUT OF RANGE <<<\n");
			}
			break;
		...
		case 9: /* Perform a Software Reset */
			...
			{
			status = wcntl445(&s_block, (byte)0x01);    /* call the routine that
