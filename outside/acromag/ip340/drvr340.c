...
*/
    ...
    struct cblk340 c_block;	 /* configuration block */
...
*/
    ...
    for(j = 0; j < 16; j++)
    {
      c_block.s_raw_buf[j] = &raw_data[j][0];	/* raw buffer start */
      c_block.s_cor_buf[j] = &cor_data[j][0];	/* corrected buffer start */
    }
...
*/
    ...
    c_block.s_count = &count[0];	/* sample counts */
    c_block.acq_mode = DI_SELECT;	/* mode */
    c_block.scan_mode = DISABLE;	/* scan mode */
    c_block.channel_enable = 0;	/* all disabled */
    c_block.threshold = 0x1ff;	/* threshold */
    c_block.ref_v = 0.0;		/* span reference */
    c_block.conv_timer0 = 0x7fff;	/* counter 0 */
    c_block.conv_timer1 = 0x80;	/* counter 1 */
    c_block.trigger = T_DISABLE;	/* no trigger */
...
*/
				...
				{
				    c_block.brd_ptr = (struct map340 *)addr;
	...
	case 5:     /* read board status command */
		...
		readstat340(&c_block); /* read board status */
	    break;
...
/*
		...
		hstatus |= SetInterruptHandler(c_block.nHandle, c_block.slotLetter, INT_REQUEST_0,
					 c_block.vector, (void*)isr_340, (void*)&hdata);
...
/*
	 if(!hflag)
	   ...
	   cnfg340(&c_block);	       / * configure the board * /
	   hflag = 0;
...
*/	 break;
	...
	case 8: /* read auto zero values */
	    ...
	    rsts340(&c_block);  /* Read Status Command also reads voltage reference */

	    calibrate340(&c_block, AZ_SELECT);   /* get auto-zero values */
	    break;
	...
	case 9: /* read calibration data values */
	    ...
	    calibrate340(&c_block, CAL_SELECT);  /* get calibration values */
	    break;
	...
	case 10: /* read raw input data values */
	    ...
	    convert340(&c_block);		      /* convert the board */
	     break;

	case 11: /* calculate corrected data value for multi channel*/
	    mccd340(&c_block);	     /* correct input data */
	    break;
...
quit_volt:
	...
	case 18:	/* write reference value */
	  ...
	  scanf("%lf",&value);
	    WriteReferenceValue( &c_block, value );
	    break;
