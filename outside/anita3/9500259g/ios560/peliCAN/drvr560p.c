...
{+D}
    ...
    ABSTRACT:       This module is the main routine for the example program
                    which demonstrates how the IOS560 Library is used.
...
*/
    ...
    memset(&c_block560[0], 0, sizeof(struct cblk560));

    c_block560[0].acr[0] = OURID;        /* Acceptance Code register */
    c_block560[0].acr[1] = OURID;        /* Acceptance Code register */
    c_block560[0].acr[2] = OURID;        /* Acceptance Code register */
    c_block560[0].acr[3] = OURID;        /* Acceptance Code register */

    c_block560[0].amr[0] = ACCEPTMASK;   /* Acceptance Mask Register */
    c_block560[0].amr[1] = ACCEPTMASK;   /* Acceptance Mask Register */
    c_block560[0].amr[2] = ACCEPTMASK;   /* Acceptance Mask Register */
    c_block560[0].amr[3] = ACCEPTMASK;   /* Acceptance Mask Register */
...
*/
    ...
    c_block560[0].cdr = 0xC8;       /* Clock Divider register (PeliCAN mode set, CBP set, CLKOUT pin off) */
    c_block560[0].ocr = 0x1A;       /* Output Control register */
    c_block560[0].mr = 0x00;        /* Mode register */
    ...
    c_block560[0].bInitialized = FALSE;/* indicate not ready to talk to IOS */
...
*/      
    ...
    while(!finished) {
        ...
        printf("\nIOS560 PeliCAN Library Demonstration  Version A\n");
...
*/  
        ...
        /* Make sure this carrier supports IOS memory space */
        ...
        do 
		{
          if(flag == 0)
		  {
            ...
            SetMemoryAddress( c_block560[0].nHandle, (long)addr2);	/* Set IOS Memory Address */
		  }
...
/*
    Get the modules base I/O address based on the Slot letter,
    and initialize the modules data structure with the returned address
*/
                  if(GetIOSAddress(c_block560[0].nHandle, c_block560[0].slotLetter, &addr) != S_OK)
                  {
                  ...
                  {
                     c_block560[0].brd_ptr = (byte *)addr;
                  }

/*
    Get the modules memory base address based on the Slot letter,
    and initialize the modules data structure with the returned address
*/
                  if(GetIOSMemoryAddress(c_block560[0].nHandle, c_block560[0].slotLetter, &addr2) != S_OK)
                  {
                  ...
                  {
                     c_block560[0].brd_memptr = (byte *)addr2;
        ...
        case 7: /* board id PROM */
              ...
              else
              {
                ...
                ReadIOSID(c_block560[current_channel].nHandle, c_block560[current_channel].slotLetter,
                ...
                printf("\nIOS Model Number:    %X",(byte)(c_block560[current_channel].idProm[5]));
...
*/
    ...
    while(!finished)
    {
        ...
        printf(" 5. Acceptance Code:    0 0x%02X 1 0x%02X 2 0x%02X 3 0x%02X\n",
              c_blk->acr[0], c_blk->acr[1], c_blk->acr[2], c_blk->acr[3]);
        printf(" 6. Acceptance Mask:    0 0x%02X 1 0x%02X 2 0x%02X 3 0x%02X\n",
              c_blk->amr[0], c_blk->amr[1], c_blk->amr[2], c_blk->amr[3]);
        printf(" 7. Bus Timing 0:       0x%02X\n",c_blk->btr0);
        printf(" 8. Bus Timing 1:       0x%02X\n",c_blk->btr1);
        printf(" 9. Output Control:     0x%02X\n",c_blk->ocr);
        printf("10. Clock Divider:      0x%02X\n",c_blk->cdr);
        printf("11. Mode:               0x%02X\n",c_blk->mr);
        ...
        case 5: /* Acceptance Code */
            printf("\nEnter register number   (0 - 3): ");
            scanf("%x",&j);
            c_blk->acr[j&3] = (byte)get_param();
            break;

        case 6: /* Acceptance Mask */
            printf("\nEnter register number   (0 - 3): ");
            scanf("%x",&j);
            c_blk->amr[j&3] = (byte)get_param();
            break;
...
eloop:
            while( i >= 0 && i < 8)
            {
              printf("%08X: %02X ", (unsigned int)&mptr[i],mptr[i] );/* print address & byte */
              scanf("%s",&buf[0]);
              buf[2] = 0;
	...
	case 4: /* View buffer */
            ...
            while( i >= 0 && i < 8)
            {
               printf("\n%08X: ", (unsigned int)&mptr[i] );/* print starting address of line */
