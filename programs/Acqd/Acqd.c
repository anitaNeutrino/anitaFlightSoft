...
int main(int argc, char **argv) {
  ...
  memset(avgScalerData,0,sizeof(avgScalerData)); 
  ...
  if(surfFirmwareVersion>=4) {
    //RJN 28/07/14 For ANITA-3 the scaler positions are all completely different
    int dac=0;
    for(dac=0;dac<SCALERS_PER_SURF;dac++) {
      logicalScalerToRawScaler[dac]=dac;
    }
    for(dac=0;dac<RAW_SCALERS_PER_SURF;dac++) {
      rawScalerToLogicScaler[dac]=dac;
    }
...
//	  
	  ...
	  if(pauseMasksSet && firstTimeThrough) {	     
		  ...
		  printf("Setting Trigger Masks (after %d secs) to %#x %#x\n",
			 pauseBeforeEvents,phiTrigMask,l1TrigMask);
		  syslog(LOG_INFO,
			 "Setting Trigger Masks (after %d secs) to %#x %#x\n",
			 pauseBeforeEvents,phiTrigMask,l1TrigMask);
...
char *turfControlActionAsString(TurfControlAction_t action) {
  ...
  case TurfClearEvent : 
    ...
    break;
  case TurfDisableTriggers : 
    string = "TurfDisableTriggers";
    break;
  case TurfEnableTriggers : 
    string = "TurfEnableTriggers";
    break;
  case SetPhiTrigMask : 
...
AcqdErrorCode_t setTurfControl(TurfControlAction_t action) {
	...
	case TurfClearAll :
	  ...
	  status=ioctl(turfioFd, TURFIO_IOCCLEARALL);	    
	    break;
	
	case TurfClearEvent : 
	    //Send TURF Clear Event

	  status=ioctl(turfioFd, TURFIO_IOCCLEAREVT);	 
	    break;


	case TurfDisableTriggers : 
	    //Send TURF Disable Triggers
	    uvalue = BitTurfTrigDisable; //Set High
	    printf("Setting TURF Clear register to: %#x -- %#x\n",TurfRegControlClear,uvalue);
	    status+=setTurfioReg(TurfRegControlClear,uvalue);	    
	    break;
	case TurfEnableTriggers : 
	    //Send TURF Enable Triggers
	    uvalue = 0; //Set Low
	    printf("Setting TURF Clear register to: %#x -- %#x\n",TurfRegControlClear,uvalue);
	    status+=setTurfioReg(TurfRegControlClear,uvalue);	    
	    break;
...
/* Load Acqd config stuff */
{
  ...
  if(status == CONFIG_E_OK) {
    ...
    surfFirmwareVersion=kvpGetInt("surfFirmwareVersion",4);
    ...
    dumpPedsAtStart=kvpGetInt("dumpPedsAtStart",0);
...
// Clears boards for start of data taking 
{
  ...
  if (setTurfControl(TurfDisableTriggers) != ACQD_E_OK) {
    syslog(LOG_ERR,"Failed to disable triggers on TURF\n") ;
    fprintf(stderr,"Failed to disable triggers on TURF\n");
  }    
  ...
  if (setTurfControl(TurfEnableTriggers) != ACQD_E_OK) {
    syslog(LOG_ERR,"Failed to enable triggers on TURF\n") ;
    fprintf(stderr,"Failed to enable triggers on TURF\n");
  }    
...
void doTurfRateSum(int flushData) {
  ...
  if(!flushData) {
	...
	sumTurf.l3RatesGated[phi]+=turfRates.l3RatesGated[phi];
    }
  }
  ...
  for(wordNum=0;wordNum<TURF_EVENT_DATA_SIZE;wordNum++) {
   ...
   dataChar=dataWord&0xff;
	...
	hdPtr->phiTrigMask+=(dataShort<<8);
      ...
      if(wordNum%2==0) {
	//First word
	hdPtr->phiTrigMaskH=dataShort;
      }
      else if(wordNum%2==1) {
	//Second word
	hdPtr->phiTrigMaskH+=(dataShort<<8);
      }	    
    }
    else if(wordNum<248) {
      //Reserved 
...
void copyTurfBank3Vals( const TurfRawBank3Struct_t * raw, TurfRateStruct_t *rate)
{
  ...
  rate->unixTime = raw->unixTime; 
  rate->ppsNum = raw->ppsNum;  
  rate->deadTime = raw->deadTime; 
  memcpy(rate->l1Rates, raw->l1Rates, sizeof(rate->l1Rates)); 
  memcpy(rate->l3Rates, raw->l3Rates, sizeof(rate->l3Rates)); 
  memcpy(rate->l3RatesGated, raw->l3RatesGated, sizeof(rate->l3RatesGated)); 
...
AcqdErrorCode_t readTurfHkData()   
{
    ...
    memset(&rawbank3, 0, sizeof(rawbank3)); 
...
int updateThresholdsUsingPID() {
  ...
  for(surf=0;surf<numSurfs;surf++) {
	...
	if(abs(theSurfHk.scaler[surf][dac]-chanGoal)>pidPanicVal)
	  wayOffCount++;
	avgScalerData[surf][dac]+= (unsigned int ) theSurfHk.scaler[trigSurf][dac];
      }
    }
  }
  if(wayOffCount>100) {
...
int checkTurfRates()
{
  ...
  int phi=0,tInd=0;
  int l1HitCount[PHI_SECTORS];
  int l1MissCount[PHI_SECTORS];  
  ...
  memset(l1HitCount,0,sizeof(int)*PHI_SECTORS);
  memset(l1MissCount,0,sizeof(int)*PHI_SECTORS);
  ...
  if(countLastTurfRates>30) {
    ...
    for(tInd=0;tInd<numLastTurfs;tInd++) {
	  ...
	  if(lastTurfRates[tInd].l1Rates[phi]>dynamicAntThresholdOverRate) 
	    l1HitCount[phi]++;
	  if(lastTurfRates[tInd].l1Rates[phi]<dynamicAntThresholdUnderRate)
	    l1MissCount[phi]++;
      }  
    }
...
//	  printf("cold channel %d -- %d\n",phi,l3MissCount[phi]);
	newPhiMask &= ~(1<<phi);
      ...
      if(l1HitCount[phi]>dynamicAntThresholdOverWindow) {
	//Got a hot channel
//	  printf("hot channel %d -- %d\n",phi,l1HitCount[phi]);
	  newL1TrigMask |= (1<<phi);
      }
      if(l1MissCount[phi]>dynamicAntThresholdUnderWindow) {
	//Got a quiet channel
//	  printf("cold channel %d -- %d\n",phi,l1MissCount[phi]);
	newL1TrigMask &= ~(1<<phi);
      }
