/*
 *
 * MODIFIED rtl_power for ANITA-4 
 *   modified by Cosmin <cozzyd@kicp.uchicago.edu> 
 *  
 *  What has changed: 
...
/** cozzyd -- add buffers to get device info */ 
...
// do we want the fewest ranges (easy) or the fewest bins (harder)?
{
	...
	/* -cozzyd took out a a bunch of crap here ... */ 
	bw_used = ANITA_RATE; 
	bw_seen = ANITA_RATE * (1-crop); 
	tune_count = (int) ceil ((upper - lower) /  (bw_seen));  
...
/* copypasted from rtl-dsr/src/convenience.c */ 
...
//			offset_tuning = 1;
	...
	rtlsdr_get_device_usb_strings( (uint32_t) dev_index, 
	...
	*/   /// these should never happen --cozzyd
